    ...
    def run_command_with_progress(
        ...
        try:
            ...
            for line in process.stdout:
                ...
                # For training commands, show more detailed progress
                if "training" in description.lower():
                    # Show important training information
                    if any(
                        ...
                        current_description = f"{description} - {line.strip()[:60]}..."
                    else:
                        current_description = f"{description} - Processing..."
                else:
                    current_description = f"{description} - {line.strip()[:50]}..."

                progress_renderable.update(task, description=current_description)
                if live:
                    # For training, show last 10 lines to see progress
                    display_lines = (
                        output_lines[-10:]
                        if "training" in description.lower()
                        else output_lines[-5:]
            ...
            if process.returncode == 0:
                ...
                # For training commands, keep the display for a bit longer
                if "training" in description.lower():
                    import time as time_module

                    time_module.sleep(2)  # Keep display visible for 2 seconds
    ...
    def test_environment(self, live: Live) -> Dict[str, bool]:
        ...
        if self.interactive:
            if not self.interactive_pause(
                "🔧 About to test environment setup (imports, smoke tests, etc.)"
            ):
                return {"Environment Tests": False}

        tests = [
            ("Python imports", f"{sys.executable} -m src.tests.test_imports"),
            ("Light smoke test", f"{sys.executable} src/tests/smoke_test_light.py"),
            (
                "Data loader",
        ...
        for i, (test_name, command) in enumerate(tests, 1):
            ...
            if self.interactive:
                if not self.interactive_pause(
                    f"Running test {i}/{len(tests)}: {test_name}"
                ):
                    # Skip remaining tests
                    for remaining_test, _ in tests[i - 1 :]:
                        results[remaining_test] = False
                        table.add_row(remaining_test, "⏭️ SKIPPED", "Skipped by user")
                    break
            ...
            if self.interactive:
                if success:
                    self.console.print(
                        f"[green]✅ {test_name} completed successfully[/green]"
                    )
    ...
    def run_quick_training(
        ...
        try:
            success, output = self.run_command_with_progress(
                ...
                timeout=600,
                live=live,
                capture_rich_output=False,  # Force streaming for training
            )
            if not success:
            ...
            # Keep the live display open for a moment to show final results
            if live:
                import time as time_module

                time_module.sleep(
                    3
                )  # Keep display visible for 3 seconds to see results
