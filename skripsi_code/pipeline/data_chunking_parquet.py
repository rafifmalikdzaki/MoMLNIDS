...
def write_chunk(output_file: Path, header: list, sample: list) -> None:
    # Convert CSV rows to a Polars DataFrame and save as Parquet
    df = pl.DataFrame({col: [row[i] for row in sample] for i, col in enumerate(header)})
    df.write_parquet(output_file)  # Save directly to Parquet
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for row in tqdm(reader, desc=f"Partitioning {input_file.stem}", leave=True):
                ...
                if len(rows) >= n_samples:
                    output_file = (
                        output_dir / f"{input_file.stem}_chunk_{chunk_count}.parquet"
                    )
