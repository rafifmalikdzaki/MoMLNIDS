...
def test_data_loading():
    ...
    try:
        ...
        console.print(
            "[success]✓ Data loading components imported successfully[/success]"
        )
        ...
        if (
            len(source_train.dataset) > 0
            and len(source_val.dataset) > 0
            and len(target_test.dataset) > 0
        ):
            console.print(
                f"[success]✓ Data loaded successfully. Source train samples: {len(source_train.dataset)}, Source val samples: {len(source_val.dataset)}, Target test samples: {len(target_test.dataset)}[/success]"
            )
        else:
...
def test_clustering_utils():
    ...
    try:
        ...
        class DummyDataset(Dataset):
            def __init__(
                self, num_samples=100, num_features=39, num_domains=3, num_classes=2
            ):
        ...
        console.print(
            "[success]✓ Pseudolabeling executed successfully and returned expected type.[/success]"
        )
...
def main():
    ...
    console.print("\n" + "[header]=" * 50 + "[/header]")
    console.print(
        f"[header]Smoke test results: {success_count}/{total_tests} tests passed[/header]"
    )

    if success_count == total_tests:
        console.print(
            "[success]🎉 All smoke tests passed! The environment is ready.[/success]"
        )
        return 0
    else:
        console.print(
            "[failure]❌ Some tests failed. Check the output above for details.[/failure]"
        )
        return 1
