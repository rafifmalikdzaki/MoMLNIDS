...
def main():
    ...
    MAX_BATCHES = config.training.get(
            "max_batches", None
    )  # Optional parameter for limiting batches
    ...
    current_label_reweighting = [
            list(x) for x in config.data.label_reweighting
    ...
    for epoch in tqdm(range(NUM_EPOCH), disable=VERBOSE):
            ...
            if USE_CLUSTER:
                if epoch % CLUSTERING_STEP == 0:
                    pseudo_domain_label = pseudolabeling(
                        ...
                        log_file=Path(config.output.logs_dir)
                        / "clustering.log",  # Use LOGS_DIR
                        epoch=epoch,
            ...
            model, optimizers, num_batches = train(
                ...
                epoch=epoch,
                num_epoch=NUM_EPOCH,
                filename=Path(config.output.logs_dir)
                / "source_trained.log",  # Use LOGS_DIR
            ...
            if global_step % EVAL_BATCH_FREQUENCY == 0:
                ...
                if val_accuracy >= best_accuracy:
                    ...
                    torch.save(
                        model.state_dict(),
                        Path(config.output.models_dir) / "model_best.pt",
                    )  # Use MODELS_DIR
